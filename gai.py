...
class IntelligentChatAssistant:
    def __init__(self, search_depth=5):
        self.client = OpenAI(api_key=json.loads(open("secret.json", "r").read())["openai"])
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", PROMPT),
            MessagesPlaceholder(variable_name="chat_history"),
            ("user", "{input}")
        ])
        self.llm = ChatOpenAI(model=MODEL, api_key=json.loads(open("secret.json", "r").read())["openai"])
    ...
    def send_query(self, event, user_input):
        ...
        if any(item in initial_response for item in fail_message):
            search_query = user_input if len(user_input) < 100 else user_input[:100]+"..."
